<?php
  ...
  if (isset($_SESSION['registrar_vehiculos'])) {
    $records = $conn->prepare('SELECT placa, marca, tipo_vehiculo FROM registrar_vehiculos WHERE placa = :placa');
    $records->bindParam(':placa', $_SESSION['registrar_vehiculos']);
    $records->execute();
    $results = $records->fetch(PDO::FETCH_ASSOC);
